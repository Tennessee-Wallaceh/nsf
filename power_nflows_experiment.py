...
def load_power_raw(data_root):
    file = os.path.join(data_root, 'power', 'data.npy')
    return np.load(file)


def prepare_power_splits(data_root):
    rng = np.random.RandomState(42)
    data = load_power_raw(data_root)
    rng.shuffle(data)
    # Remove two columns
...
def main(args):
    ...
    if data_root is None:
        raise RuntimeError('DATAROOT environment variable is not set.')
    train_array, val_array, test_array = prepare_power_splits(data_root)
